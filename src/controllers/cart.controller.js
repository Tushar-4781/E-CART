...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  if(req.body.quantity>0){
    let USER = await cartService.updateProductInCart(req.user,req.body.productId,req.body.quantity);
    if(USER){
      res.status(200).send(USER);
    }
  }
  else{
    let USER = await cartService.deleteProductFromCart(req.user,req.body.productId);
    if(USER){
